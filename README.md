...
1. **Misinformation Detection** — cluster public comments into misunderstanding "trends", cross-check them against the real legal provision, and issue a correction.
...
| 6 | **Detect trends, misunderstandings, misinformation & communications risk** | Trend clustering + correction from the real article |
...
- **Misinformation detection (Req. 6)** — comments that contradict the linked provision are clustered into misunderstanding trends with a correction drawn from the real article.
...
|---|---|---|
| **Dashboard** | http://localhost:8000/ | 3 tabs: Misinformation alerts · Cited Q&A · Document search |
...
| `GET` | `/misconception/{misc_id}` | A misunderstanding + the correcting provision + illustrative posts |
